...
@implementation ZCAdditionalView
{
...
- (id)initWithFrame:(CGRect)frame withSelectAddtions:(NSArray *)array
{
    ...
    if (self) {
        ...
        _tableV = [[UITableView alloc] initWithFrame:CGRectMake(5, 100,CGRectGetWidth(self.frame)-7, CGRectGetHeight(self.frame)-200) style:UITableViewStylePlain];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    [button setTitle:[[[_dataArray objectAtIndex:section] lastObject] objectForKey:@"RGRPDES"] forState:UIControlStateNormal];
    button.tag=section;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BSAdditionCell *cell=[tableView dequeueReusableCellWithIdentifier:cellName];
    if (!cell) {
        cell=[[BSAdditionCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellName];
    }
    cell.selectionStyle=UITableViewCellSelectionStyleNone;
    [cell setContent:[[_dataArray objectAtIndex:indexPath.section] objectAtIndex:indexPath.row]];
    return cell;
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    BSAdditionCell *cell=(BSAdditionCell *)[_tableV cellForRowAtIndexPath:indexPath];
    if ([[cell.info objectForKey:@"SELECT"]boolValue]==YES) {
        [cell.info setValue:[NSNumber numberWithBool:NO] forKey:@"SELECT"];
        for (NSDictionary *dict in _selectArray) {
            if ([[dict objectForKey:@"ITCODE"] isEqualToString:[cell.info objectForKey:@"ITCODE"]]) {
                [_selectArray removeObject:dict];
                break;
            }
        }
        [cell setContent:cell.info];
    }else
    {
        [cell.info setValue:[NSNumber numberWithBool:YES] forKey:@"SELECT"];
        [_selectArray addObject:cell.info];
        [cell setContent:cell.info];
    }
}
...
{
    //获取全部的附加项
    NSMutableArray *ary = [NSMutableArray arrayWithArray:[[BSDataProvider sharedInstance] ZCgetAdditions]];
    ...
    for (NSDictionary *dic in _selectArray) {
        ...
        for (int i=0;i<[ary count];i++) {
            ...
            for (int j=0;j<[array count];j++) {
                ...
                if ([[dic objectForKey:@"ITCODE"] isEqualToString:[dict objectForKey:@"ITCODE"]]&&[[dic objectForKey:@"RGRP"] isEqualToString:[dict objectForKey:@"RGRP"]]) {
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex==1) {
        ...
        NSMutableDictionary *dict=[[NSMutableDictionary alloc] initWithObjectsAndKeys:@"自定义",@"RGRPDES",[alertView textFieldAtIndex:0].text,@"DES",[NSString stringWithFormat:@"%.2f",[[alertView textFieldAtIndex:1].text floatValue]],@"AMT",@"RGRP",@"RGRP",@"",@"ITCODE",[NSNumber numberWithBool:YES],@"SELECT",nil];
        ...
        for (NSArray *array in ary) {
            ...
            for (NSDictionary *dic in array) {
                ...
                if ([strITCODE rangeOfString:searchText].location!=NSNotFound ||
                    [strDES rangeOfString:searchText].location!=NSNotFound||[strINIT rangeOfString:searchText].location!=NSNotFound){
