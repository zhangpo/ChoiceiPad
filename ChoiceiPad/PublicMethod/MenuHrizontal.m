...
-(void)createMenuItems:(NSArray *)aItemsArray{
    ...
    for (NSDictionary *lDic in aItemsArray) {
        ...
        vButton.titleLabel.font=[UIFont boldSystemFontOfSize:20];
...
-(void)moveScrolViewWithIndex:(NSInteger)aIndex{
    ...
    if (vButtonOrigin >= 768) {
        if ((vButtonOrigin + 180) >= mScrollView.contentSize.width) {
            [mScrollView setContentOffset:CGPointMake(mScrollView.contentSize.width - 768, mScrollView.contentOffset.y) animated:YES];
            return;
        }
